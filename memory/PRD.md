...
## 2026-08-22 — 3 görev daha: sync.php v39 upsert + Geçmiş gün doğrulama + Haftalık Trend ✅
1. **sync.php v39 (POS kalıcı kopya önleme)**: `dataset_delta_push` içinde `row_uid_hash` artık params_hash İÇERMEZ → `sha256(tenant|dataset|row_key)`. Eski formül her push'ta farklı paramsHash ürettiğinden upsert hiç eşleşmiyordu (140K ölü kopyanın kök nedeni). Delete uid'i de aynı stabil formüle geçirildi; ON DUPLICATE'e `params_hash=VALUES(params_hash)` eklendi. Satır ~888'deki backfill BİLEREK eski formülde bırakıldı (eski kopyalar UNIQUE çakışması yaratır; günlük temizlik onları ayıklıyor — yorumla belgelendi). KULLANICI AKSİYONU: yeni sync.php hosting'e yüklenmeli.
...
## 2026-09-19 — v18: 200K ürünlü müşteri (a6491c78) — saatlik ürün detayı 18 sn → 1 sn, dashboard 2 sorgu, Günlük Ürün Satışları ✅
- **Kök neden:** `dataset_cache_rows.hourly_stock_detail` satırları tenant için 9.952 satır / **531 MB** (her satır saat×lokasyon üst kaydı + iç içe URUNLER). Sorgular `row_json LIKE '%"TARIH":"…"%'` ile TÜM blobları tarıyordu → `/hourly-detail` 18 sn, `/hourly-detail-full` 18,5 sn → grafiğe dokununca ürünler gelmiyordu.
- **Fix:** günün `params_hash`'i `dataset_cache` üst kaydından çözülür (`_hourly_gun_hashleri`, services/dataset_cache.py) ve satırlar `(tenant_id, dataset_key, params_hash)` indeksiyle okunur. Uygulandığı yerler: `lookup_rows_dataset` (tek saat / tam gün ham / agregasyon yolları) ve `/hourly-detail` STEP 1. Hash yoksa eski LIKE yolu.
- **Dashboard tek gün:** `fetch_day_datasets()` — 11 veri seti için 22 sorgu yerine 1 meta (updated_at ≥ gün) + 1 blob akışı; `fetch_dataset(row=…)` ile aynı son işlem. 2,7-4,9 sn → 1,0-1,7 sn.
- **Yeni:** `GET /api/data/gunluk-urun-satis?tenant_id&tarih&lokasyon_id?` — günde her üründen kaç adet satıldı (STOK_ID toplaması, MIKTAR/TUTAR/SAAT_SAYISI/LOKASYONLAR). Dashboard'a **"Günlük Ürün Satışları"** bölümü (`DailyProductSalesSection.tsx`): arama, Adet/Tutar sıralama, göreli çubuk, 10'arlı yükleme; tarih + şube filtresine bağlı. testID: daily-product-sales, daily-product-search, daily-product-sort-adet/tutar, daily-product-more.
- Sürüm 1.0.51 (iOS 55 / Android 55); `/api/sistem-durum` → `2026-09-19-v18-saatlik-indeks`.
- v18 ek: `close_pools()` süre sınırı + terminate, shutdown'da kendi arka plan görevleri iptal (uvicorn reload / Railway SIGTERM'de "Waiting for application shutdown" askısı giderildi). Testler: `tests/test_v18_gunluk_urun_satis.py` 10/10, iteration_22 frontend PASS.

## 2026-09-19 — v19: Büyük stok listesi akış modu + Ürün Detayı + Haftalık Ürün Trendi ✅
- **Stok listesi (200K ürün) kök neden:** a6491c78 için `dataset_cache_pages`'te HİÇ stock_list yok, sync_logs'ta stock_list kaydı yok → POS istemcisi tüm fiyat adlarının satırlarını RAM'de toplayıp hash'lerken (yüz binlerce dict) hiç push'a ulaşamıyor. Ayrıca backend `get_dataset_items` tüm listeyi RAM'e alıyordu (Railway 250 MB → OOM).
  - **client.py:** `_stock_list_is_big()` (ilk fiyat adı > 40K satır / tahmin > 80K → akış modu, 1 sa önbellek) + `_push_stock_list_streaming()` — fiyat adı bazında çekip sayfa sayfa hemen gönderir (dataset_page_begin total_parts=1 geçici; commit gerçek parça sayısı — sync.php sadece alınan==total kontrolü yapar), zincir hash, snapshot elle yazılır; akış modunda tam gönderim en fazla 6 saatte bir (`STOCK_LIST_STREAM_MIN_INTERVAL_SEC`). Müşteri POS'unda client güncellenmeli.
  - **backend:** `services/dataset_cache.py` → `BUYUK_VERI_ESIGI=25_000`, `dataset_satir_sayisi()`, `stream_dataset_items()` (sayfa sayfa, isteğe bağlı `data_json LIKE` ön süzgeci). `/stock-list` büyük tenant'ta akış yolu (`_source: mysql_stream`; süzgeçsiz istekte sayfa dolunca durur; arama ASCII 3+ karakterse LIKE ön süzgeci), `/barcode-price` büyük tenant'ta LIKE ile arar. ea5231b8 (28K): sayfa 2 → 1,0 sn, arama → 4 sn.
