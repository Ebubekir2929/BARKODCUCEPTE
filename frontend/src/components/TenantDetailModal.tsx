...
export const TenantDetailModal: React.FC<{
  ...
  const productsByBranch = useMemo(() => {
    ...
    // Sort each branch's products by amount DESC
    ...
    >
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
        ...
        >
          ...
          {/* Top stock per branch — user-requested: "stoklardan en çok şu kadar satılmış" */}
          {Object.keys(productsByBranch).length > 0 && (
            <View style={[styles.sectionBox, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <Ionicons name="cube-outline" size={16} color={color} />
                <Text style={[styles.sectionTitle, { color: colors.text }]}>Şube Bazlı En Çok Satan Stoklar</Text>
              </View>
              {Object.entries(productsByBranch).map(([branch, products]) => {
                ...
                return (
                  <View key={branch} style={{ marginBottom: 14, borderLeftWidth: 3, borderLeftColor: bColor, paddingLeft: 10 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                        <Ionicons name="location" size={12} color={bColor} />
                    ...
                    {top5.map((p, i) => (
                      <View key={p.name + i} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 6, borderBottomWidth: i < top5.length - 1 ? 1 : 0, borderBottomColor: colors.border }}>
                        <View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: bColor + '22', alignItems: 'center', justifyContent: 'center', marginRight: 8 }}>
                          <Text style={{ color: bColor, fontSize: 10, fontWeight: '800' }}>#{i + 1}</Text>
                        </View>
          ...
          {/* Saatlik Fiş Sayısı — per tenant, per hour */}
